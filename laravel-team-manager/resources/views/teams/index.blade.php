<x-layouts::public :title="__('Teams')">
    <div class="flex w-full flex-1 flex-col gap-6">
        <div class="flex flex-col gap-1">
            <h1 class="text-2xl font-semibold text-neutral-900 dark:text-white">Teams</h1>
            <p class="text-sm text-neutral-600 dark:text-neutral-400">Browse all available teams.</p>
        </div>
        ...
        <ul class="flex flex-col gap-3 list-none">
            @forelse ($teams as $team)
                <li class="border border-neutral-200 dark:border-neutral-700 rounded-lg overflow-hidden">
                    <a href="{{ route('teams.show', $team) }}" class="block px-4 py-4 text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 transition-colors duration-150 text-sm font-medium hover:bg-neutral-50 dark:hover:bg-neutral-900">
                        {{ $team->name }}
                    </a>
                </li>
            @empty
                <li class="text-center py-8 text-sm text-neutral-600 dark:text-neutral-400">
                    No teams are available. Try adjusting your search.
                </li>
            @endforelse
        </ul>
